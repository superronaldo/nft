...
const ConnectWalletButton = (props) => {
  ...
  const renderButton = () => {
    if (!account) {
      return (
        <button type="button" className="btn" style={{ backgroundColor: "#17cd2d", fontFamily: 'Gugi', letterSpacing: '0.2em', marginBottom: 24, marginTop:20, }} onClick={onPresentConnectModal} {...props}>
          Connect Wallet
        </button>
      )
    }
      ...
      <div style = {{ marginBottom: 24, marginTop:20,}}>
        <StyledAccountBack {...props}>
          {fetchStatus !== FetchStatus.SUCCESS ? (
            ...
            <StyledAccount color='black'>{`${formatBigNumber(balance, 2)}MATIC`}</StyledAccount>
          )}
          <StyledBalance color='black' style={{backgroundColor:'#17cd2d'}} onClick={logout}>{`${account.substring(0, 6)}...${account.substring(38, 42)}`}</StyledBalance>
        </StyledAccountBack>
      </div>
    )
  }
